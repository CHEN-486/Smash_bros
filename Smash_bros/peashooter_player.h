...
public:
	Peashooter() 
	{
		...
		timer_attack_ex.set_wait_time(attack_ex_durtion);
		timer_attack_ex.set_one_shot(true);
		timer_attack_ex.set_callback([&]() {is_attacking_ex = false; });

		timer_attack_ex_cd.set_wait_time(100);
		timer_attack_ex_cd.set_callback([&]() {spawn_pea_bullet(speed_pea_ex); });

		timer_attack_cd.set_wait_time(150);
	...
	void on_update(int delta)
	{
		...
		if(is_attacking_ex)
		{
			main_camera.shak(5,100);
			timer_attack_ex.on_update(delta);
			timer_attack_ex_cd.on_update(delta);
		}
	}
...
private:
	void spawn_pea_bullet(float speed)
	{
		...
		Vector2 pos;
		pos.x = is_facing_right ?
			position.x + size.x - bullet->get_size().x / 2 :
			position.x - bullet->get_size().x / 2;
		pos.y = position.y;
		bullet->set_position(pos);
		bullet->set_velocity(is_facing_right ? speed : -speed, 0);
...
private:
	...
	const int attack_ex_durtion = 2500;
private:
	Timer timer_attack_ex;//特殊攻击计时
	Timer timer_attack_ex_cd;//特殊攻击子弹间隔计时
};
#endif
